...
class ProgramScanDialog(QDialog):
    def __init__(self, image_manager=None, parent=None, scaling=None, cache_file=None):
        ...
        self.setModal(True)
        self.setFixedSize(s.scale(700), s.scale(650))
        self.setStyleSheet(f"""
            QDialog {{ background-color: #1a1a1a; }}
            QLabel {{ color: white; font-size: {s.scale_font(16)}px; }}
            QLineEdit {{ background-color: #2a2a2a; color: white; border: 2px solid #444; padding: {s.scale(8)}px; border-radius: {s.scale(8)}px; font-size: {s.scale_font(14)}px; }}
            QListWidget {{ 
            ...
            QListWidget::item {{ 
                padding: {s.scale(8)}px; 
                border-radius: {s.scale(4)}px;
        ...
        if refresh_icon_path.exists():
            ...
            self.refresh_btn.setIconSize(QSize(s.scale(20), s.scale(20)))
        else:
            self.refresh_btn.setText("↻")
            self.refresh_btn.setStyleSheet(f"""
                QPushButton {{
                    font-size: {s.scale_font(20)}px;
                    font-weight: bold;
                }}
            """)
        
        self.refresh_btn.setFixedSize(s.scale(40), s.scale(40))
        ...
        self.progress_label.setStyleSheet(f"color: #888; font-size: {s.scale_font(12)}px;")
        ...
        self.list_widget.setIconSize(QSize(s.scale(32), s.scale(32)))
        layout.addWidget(self.list_widget)

        self.info_label = QLabel("Select which programs to add (Ctrl/Shift for multiple)")
        self.info_label.setStyleSheet(f"color: #aaa; font-size: {s.scale_font(13)}px;")
