...
class TVLauncher(QMainWindow):
    ...
    def init_joystick(self):
        try:
            ...
            if pygame.joystick.get_count() > 0:
                ...
                # ✨ SPOSTATO QUI DENTRO (solo se joystick esiste)
        ...
        try:
            ...
            if count > 0:
                if self.joystick is None:
                    ...
                    # ✨ QUI È OK (joystick appena connesso)
                    if self.volume_manager:
                        self.volume_manager.set_joystick(self.joystick)    
            else:
                if self.joystick is not None:
                    ...
                    # ✨ SPOSTATO QUI DENTRO (disconnessione confermata)
                    if self.volume_manager:
                        self.volume_manager.set_joystick(None)
            ...
            if self.joystick is not None:
                ...
                # ✨ AGGIUNGI ANCHE QUI (errore = disconnessione)
                if self.volume_manager:
                    self.volume_manager.set_joystick(None)
    ...
    def poll_joystick(self):
        ...
        try:
            if hasattr(self, 'volume_manager') and self.volume_manager and self.volume_manager.volume_mode_active:
            # Consuma comunque gli eventi per evitare accumulo
                for event in pygame.event.get():
                    pass
                return 
            # ✨ CONSUMO EVENTI invece di solo pump (include JOYDEVICEREMOVED)
            for event in pygame.event.get():
                pass  # Consuma e scarta tutti gli eventi
    ...
    def save_config(self):
        #  NUOVO: Pulisci i dati prima di salvare
        clean_apps = []
        ...
        with open(self.config_file, 'w') as f:
            json.dump({
                'apps': clean_apps,  # Usa la lista pulita
    ...
    def update_background(self):
        if self.background_image and Path(self.background_image).exists():
            ...
            # Default gradient: dark subtle per TV/media center
            self.setStyleSheet("""
                    QMainWindow {
                    background-color: #2a2a2a;
                }
    ...
    def _on_app_ready_from_scan(self, app_data):
        ...
        # ✨ NUOVO: Crea una copia pulita senza oggetti Qt
        clean_app_data = {
        ...
        if isinstance(clean_app_data['icon'], QPixmap):
            # Se per qualche motivo è un QPixmap, usa il path
...
def main():
    ...
    launcher = TVLauncher()
    volume_manager = install_volume_control(launcher.scaling,launcher)
    launcher.show()
    launcher.setFocus()
